...
`0.1.0`은 아직 출시하지 않은 개발 버전이다. 구간 단위 selection·editing·verification 계약, edit별 부분 원복 finalizer, digest 연결과 본문 비저장 검사는 구현되어 있다. 알려진 선정 진단과 과거의 위험한 후보를 거부하는 회귀 검사는 각각 세 번 통과했다. 사용자에게 노출된 내부 구현 은유도 특정 단어 목록이 아니라 문맥으로 판정하며, 일반 독자에게 모호한 표현과 실제 영수증·기술 식별자 같은 정상 용례를 함께 둔 10개 대조 사례를 세 번 모두 구분했다.

새로운 안전한 편집 후보를 충분히 골라 내는 재현성은 아직 릴리스 기준에 미치지 못한다. 두 번째 진단 시도에서 세 선정자의 편집 대상 선택은 각각 6/11, 11/11, 9/11이었다. 이 실패 결과는 `evals/cycles/0.1.0-rc2/diagnostic/semantic-regression/new-candidates/`에 보존했다. 새 비공개 holdout을 이용한 정식 평가는 시작하지 않았으며, MCP 통합 저장소 편입과 배포도 하지 않았다. 이후 작업은 합격선을 낮추지 않고 이 편차를 줄이는 고도화 단계로 진행한다.
